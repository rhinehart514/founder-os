...
How to read and render every /founder view. The source of truth for templates, conditional rendering, snapshot protocol, opinion logic, and pattern detection.

## Dashboard Zones (default view)

Portfolio zone (conditional) + four project zones. Each dense, visual, earns its space.

### Zone 0: Portfolio Summary (conditional)

Only renders when `~/.founder-os/portfolio.yml` exists. Shows all ideas at a glance before diving into the current project.

```
  ◆ founder-os  ·  [founder name]
  ⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯

  portfolio    3 ideas  ·  2 active  ·  1 killed
  accuracy     65%  (13/20 graded)  ·  target: 50-70%

  REB              validating     12d     GBP integration 80% done
  SaaS Toolkit     researching    3d      competitor analysis in progress

  killed: MarketMachine (2026-03-10) → learned: 13-agent research overkill for validation

  ⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯
```

- Read `~/.founder-os/portfolio.yml` for idea list, stages, statuses
- Read `~/.claude/knowledge/predictions.tsv` for accuracy
- Active ideas: show name, stage, days in stage, one-line status
- Killed ideas (last 3): name, kill date, learning extracted
...
- Value hypothesis from `config/founder.yml` → `value.hypothesis`
- User from `config/founder.yml` → `value.user`
...
- No portfolio.yml → skip Zone 0 entirely
...
- No eval-cache but assertions exist → run `founder eval . --score` inline
- Skip zones with no data entirely — don't show empty state

## Dashboard Snapshot Protocol

After every `/founder` run (default view only), save current state to `.claude/cache/founder-snapshots.json`:
  ...
  "snapshots": [
    {
      ...
      "bottleneck": "learning",
      "portfolio_summary": {"total": 3, "active": 2, "killed": 1}
    }
  ]
}
```
...
## Health View (`/founder health`)

System health audit. Is founder-os itself working?
...
```
  ◆ founder health  ·  grade: **B**

  hooks        8/8 scripts present  ✓
  agents       4/6 producing todos  · silent: reviewer, market-analyst
  skills       12/18 with assertions  · unmeasured: calibrate, skill, onboard, research, ship
...
## Progress View (`/founder progress`)
...
Default: 7 days. Accepts argument: `/founder progress 30` for 30 days.

ASCII line charts with `●` data points, `━` connections, `┤└──` axes. Feature maturity uses `░` planned `▓` building `█` working `●` polished.

---

## Help View (`/founder help`)

Run `scripts/skill-catalog.sh` for raw data. Render grouped by phase:

1. **ideate** — /decide, /ideate
2. **validate** — /research, /strategy, /product, /money
3. **build** — /plan, /go, /push, /todo
...
## System View (`/founder system`)

Internals: version, mode, counts, calibration status, agents with models, hooks with events.
...
If `plan.yml` exists and was updated within 24 hours, use its `bottleneck_feature`. Otherwise compute independently but note: "Opinion based on /founder heuristic — run /plan for authoritative diagnosis."
